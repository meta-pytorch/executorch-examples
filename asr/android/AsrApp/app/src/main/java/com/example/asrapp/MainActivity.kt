...
import com.example.asrapp.ui.theme.AsrTheme
...
class MainActivity : ComponentActivity(), AsrCallback {
    ...
    companion object {
        ...
        // Whisper token lengths to remove from transcription output
    ...
    private var currentModelType by mutableStateOf(ModelType.PARAKEET)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.setAppStorageDirectory("${filesDir.absolutePath}/asr")
        viewModel.initialize("/data/local/tmp/asr", modelType = ModelType.PARAKEET)
                    ...
                    when (currentScreen) {
                        ...
                        Screen.SETTINGS -> {
                            ModelSettingsScreen(
                                ...
                                showModelTypeSelector = true,
                                onModelTypeChanged = { type -> currentModelType = type }
                            )
                        }
                    }
                }
    ...
    private fun applyDownloadedModelPaths() {
        ...
        val preprocessorPath = downloadViewModel.getPreprocessorPath()
        if (preprocessorPath.isNotEmpty()) {
            viewModel.selectPreprocessor(preprocessorPath)
        } else {
            viewModel.clearPreprocessor()
        }
        viewModel.selectModelType(preset.modelType)
    ...
    /**
     * Run inference on a WAV file from disk.
     */
    private fun runFromFile(wavFilePath: String) {
        buttonEnabled = false
        ...
        Thread {
            try {
                when (currentModelType) {
                    ModelType.WHISPER -> runWhisperOnWavFile(wavFilePath)
                    ModelType.PARAKEET -> runParakeetOnWavFile(wavFilePath)
                }
            } catch (e: Exception) {
    ...
    /**
     * Run inference on the recorded audio file.
     */
    private fun runInference() {
        val wavFile = File(getExternalFilesDir(null), "audio_record.wav")
        when (currentModelType) {
            ModelType.WHISPER -> runWhisperOnWavFile(wavFile.absolutePath)
            ModelType.PARAKEET -> runParakeetOnWavFile(wavFile.absolutePath)
        }
    }
    ...
    private fun runWhisperOnWavFile(wavFilePath: String) {
        ...
        Log.v(TAG, "Starting Whisper transcribe for: $wavFilePath")
        runOnUiThread {
        ...
        Log.v(TAG, "Finished Whisper transcribe in ${elapsedSeconds}s")
    ...
    private fun runParakeetOnWavFile(wavFilePath: String) {
        ...
        Log.v(TAG, "Starting Parakeet transcribe for: $wavFilePath")
        runOnUiThread {
            statusText = "Transcribing..."
        }
        val startTime = System.currentTimeMillis()
        ...
        val elapsedTime = System.currentTimeMillis() - startTime
        val elapsedSeconds = elapsedTime / 1000.0
        Log.v(TAG, "Finished Parakeet transcribe in ${elapsedSeconds}s")
        ...
        when {
            ContextCompat.checkSelfPermission(
                ...
                try {
                    ...
                    if (currentModelType == ModelType.WHISPER) {
                        buttonText = "Recording... (30s)"
                        buttonEnabled = false

                        // Schedule automatic stop after 30 seconds for Whisper
        ...
        Surface(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                if (modelSettings.isValid()) {
                    Text(
                        text = "Type: ${modelSettings.modelType.name.lowercase()
                    ...
                    if (modelSettings.modelType == ModelType.WHISPER) {
