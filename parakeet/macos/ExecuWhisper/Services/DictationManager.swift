...
final class DictationManager {
    ...
    private func stopAndPaste(trigger: String) async {
        ...
        do {
            ...
            dictationLog.info("Dictation produced outputLength=\(result.outputText.count) rawLength=\(result.rawText.count)")
            defer {
                dismissPanel()
                state = .idle
            }
            ...
            if Self.checkAccessibility(prompt: false) {
                dictationLog.info("Accessibility permission present; attempting auto-paste")
                pasteViaKeyEvent()
            } else {
                dictationLog.error("Accessibility permission missing during auto-paste")
    ...
    private func pasteViaKeyEvent() {
        guard let keyDown = CGEvent(
            ...
            dictationLog.error("Failed to create Cmd+V events for paste")
            return
        }
        ...
        dictationLog.info("Posted synthetic Cmd+V")
    }
